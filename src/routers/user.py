...
@router.get("/{id}/results", response_model=list[ProcessedResult])
async def get_user_results(
    ...
    try:
        results = await controller.list_results(user_id=id, skip=skip, limit=limit)
        return ApiResponseFormatter.success(results=[r.model_dump() for r in results])
    except Exception as err:
